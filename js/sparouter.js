...
var sparouter = new function () {
	...
	Sparouter.prototype.start = function () {
		...
		function handleBackLinks() {
			$("a[data-back]").on('click', function(e) {
				e.preventDefault();
				var options = $(this).attr("data-page-options");
				var effect = $(this).attr("data-transition-effect");
		...
		//	console.log("onhashchange: ", +document.location + ", hash:" + document.location.hash + ", oldurl:" + event.oldURL + ", newurl:" + event.newURL + ", state: " + JSON.stringify(event.state));
